...
class FrequencyDomainLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # 1. 简单的 Mask 处理：将无效值(<=0) 设为 0，避免影响 FFT
        # 注意：FFT 对噪声和突变敏感，最好只计算有效区域，但 FFT 需要规则矩形。
        # 策略：将无效区域填 0 (或均值)，这里简单填 0
        ...
        # 3. 计算实部和虚部的 L1 损失 (也就是幅度谱和相位谱的综合约束)
        loss_real = self.l1_loss(torch.real(fft_pred), torch.real(fft_gt))
        loss_imag = self.l1_loss(torch.imag(fft_pred), torch.imag(fft_gt))
