...
class Cube2Equirec(nn.Module):
    ...
    def forward(self, x, mode='bilinear'):
        ...
        for i in range(6):
            ...
            # sample = F.grid_sample(now, XY, mode=mode, align_corners=True)[..., 0, :]     # 原作者代码
            sample = F.grid_sample(now, XY, mode=mode, align_corners=True, padding_mode='border')[..., 0, :]    # 20251226 修改 以解决深度图存在小黑点的问题
