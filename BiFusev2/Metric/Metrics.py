...
class MovingAverageEstimator(object):
    ...
    def __call__(self):
        ...
        for name in self.field_names:
            # avg_results[name] = self.results[name] / self.count   # 这里返回了 numpy 类型 部分指标在终端对应输出时会有 np.float32(...) 的包裹
            # 【20251220 修改】强制转换为 Python 原生 float
            avg_results[name] = float(self.results[name] / self.count)

        return avg_results

    # 20251110 deepseek 修改的
    # def __call__(self):
    #     avg_results = {}
    #     for name in self.field_names:
    #         if self.count == 0:
    #             # 当count为0时，返回NaN或其他默认值
    #             avg_results[name] = float('nan')
    #             print(f"⚠️ Warning: Metric '{name}' division by zero (count=0), setting to NaN")
    #         else:
