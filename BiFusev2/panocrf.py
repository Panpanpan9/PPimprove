...
# from .pano_layers import *    # 注释掉 为了适配我的 PanoLayers.py

from argparse import Namespace

# from .models import register  # 注释掉 为了适配我的 PanoLayers.py
from .PanoLayers import *   # 202601xx 【暂时还未调整：将骨干网络升级为 EfficientNet-B5】
########################################################################################################################
...
class EfficientNetEncoder(nn.Module):
    def __init__(self):
        ...
        # 这里 pretrained=True 即表示加载在 ImageNet 上预训练好的权重 EfficientNet-B5
...
# 【20260104 修改】注释掉底部的注册函数（因为它依赖 register）： 找到文件底部的 @register('panocrf') 部分，将其全部注释掉，因为 BiFusev2 不使用这就 registry 机制。
# @register('panocrf')
